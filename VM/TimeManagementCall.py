...
def collect_New_Twits(res):
    try:
        ...
        #
        df = (pd.io.json.json_normalize(res['messages'])[{'id','body','created_at','entities.sentiment.basic','symbols'}])
        ...
        df = (pd.io.json.json_normalize(res['messages'])[{'id','body','created_at','entities.sentiment','symbols'}])
...
def gather_twits():
    ...
    for symbol in stockSymbol:
        ...
        if pd.io.json.json_normalize(response['messages']).empty:
            time.sleep(15)
...
def MakeBarChart(bullishCount,bearishCount,noneCount,symbol,timeSeries, script, images):
    data = [noneCount, bearishCount, bullishCount]
#    tag = []
#    for key in tags.keys():
#        data.append(tags[key])
#        tag.append(key)
    
    df = pd.DataFrame({'Tags':['None', 'Bearish', 'Bullish'], 'Counts': data})
    ax = df.plot.barh('Tags', 'Counts', color=['y', 'r', 'g'], fontsize=15, figsize=(11,10), legend=False)
    ...
    if days == 'lastWeek':
        ...
        for row in df.tag:
            if row == 'Bullish':
                ...
                noneCount += 1
        MakeBarChart(bullishCount,bearishCount,noneCount,symbol,'LastWeek', script, images)
    elif days == 'lastMonth':
        lastmonthdate = pd.to_datetime('today').floor('D') - timedelta(30)
        df = df[df.Date >= lastmonthdate]
        for row in df.tag:
            if row == 'Bullish':
                ...
                noneCount += 1
        MakeBarChart(bullishCount,bearishCount,noneCount,symbol,'LastMonth', script, images)
    elif days == 'lastYear':
        lastyeardate = pd.to_datetime('today').floor('D') - timedelta(365)
        df = df[df.Date >= lastyeardate]
        for row in df.tag:
            if row == 'Bullish':
                ...
                noneCount += 1
        MakeBarChart(bullishCount,bearishCount,noneCount,symbol,'LastYear', script, images)
    else:
        for row in df.tag:
            if row == 'Bullish':
                bullishCount += 1
                ...
                noneCount += 1
        MakeBarChart(bullishCount,bearishCount,noneCount,symbol,'AllTime', script, images)
